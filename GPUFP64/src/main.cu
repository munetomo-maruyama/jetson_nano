...
// Device Kernel
//-----------------
__global__ void Device_Kernel(double *buf, const double a, const double b)
{
...
int main(void)
{
    ...
    double *hBuf;
    if ((hBuf = (double*)malloc(sizeof(double) * DATA_SIZE)) == NULL) exit(EXIT_FAILURE);
    //
    ...
    {
        hBuf[i] = (double)((rand() % 10000) - 5000) / 1000.0f;
    }
    //
    // Allocate Device Buffer
    double *dBuf;
    CHECK(cudaMalloc((double **) &dBuf, sizeof(double) * DATA_SIZE));
    CHECK(cudaMemcpy(dBuf, hBuf, sizeof(double) * DATA_SIZE, cudaMemcpyHostToDevice));
    //
    // Grids and Blocks
    ...
    //
    // Display Result
    CHECK(cudaMemcpy(hBuf, dBuf, sizeof(double) * DATA_SIZE, cudaMemcpyDeviceToHost));
